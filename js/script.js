...
async function triggerTopologyScan() {
    if (window.SMARTCAMPUS_CLOUD) return;
    const baseUrl = getTopologyApiBase();
    try {
...
function startTopologyRealtime() {
    ...
    if (!window.SMARTCAMPUS_CLOUD && typeof io === 'function') {
...
async function initScanSettings() {
    ...
    if (settings?.available === false) {
        if (statusEl) statusEl.textContent = 'Manage live scanning on the campus computer.';
        return;
    }
